...
class ForceUtil @Inject() (configuration: Configuration, ws: WSClient) (implicit ec: ExecutionContext) {
  ...
  def processHistory(env: String, sessionId: String, body: String): Future[JsValue] = {
    restUrl(env, sessionId).flatMap { restUrl =>
        ws(restUrl + "tooling/executeAnonymous", sessionId).withQueryString("anonymousBody" -> body).get().flatMap(createdResponseToJson)
    }
  }
    ...
    restUrl(env, sessionId).flatMap { restUrl =>
      ws(restUrl + "query", sessionId).withQueryString("q" -> "SELECT ApiToken__c,APIURL__c,GatewayToken__c,graphdburl__c from HardingPoint__c").get().flatMap { response =>
        response.status match {
          ...
          case _ => Future.failed(new Exception("Custom Settings HardingPoint__c does not Exit. Please see instructions"))
        }
      }
    }
  }
